...
namespace Skidbladnir.Storage.WebDav.Sample
{
    public class StartupModule : RunnableModule
    {
        ...
        {
            var storageConfiguration = Configuration.Get<WebDavStorageConfiguration>();
            services.AddWebDavStorage(storageConfiguration);
        }
        ...
        public override async Task StartAsync(IServiceProvider provider, CancellationToken cancellationToken)
        {
            ...
            _storage = provider.GetService<IStorage<WebDavStorageInfo>>();
            try
            {
